...
public class NodoController {
    ...
    public ResponseEntity<?> getNodos()
            throws Exception {
    ...
    @SuppressWarnings({"rawtypes", "unchecked"})
    @RequestMapping(value = "/nodosActivos", method = RequestMethod.GET, produces = "application/json")
    public ResponseEntity<?> getNodosActivos()
            throws Exception {

        log.info("Buscando todos los nodos. ");
        List<Nodo> nodos = nodoService.getNodosActivos();
        return new ResponseEntity(nodos, HttpStatus.OK);
    }
