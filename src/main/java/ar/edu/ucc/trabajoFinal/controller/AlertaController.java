...
public class AlertaController {
	...
	/**
	 * Busca todas las alertas que generaron la alerta visible.
	 * @param idAlerta
	 * @return
	 * @throws Exception
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	@RequestMapping(value = "/detalleAlerta/{idAlerta}", method = RequestMethod.GET, produces = "application/json")
	public ResponseEntity<?> getDetalleAlerta(@PathVariable("idAlerta") Long idAlerta) throws Exception {
		List<Alerta> alertas = alertaService.getDetalleAlerta(idAlerta);
		return new ResponseEntity(alertas, HttpStatus.OK);
	}
}
