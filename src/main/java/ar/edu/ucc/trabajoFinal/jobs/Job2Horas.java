...
public class Job2Horas extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
        ...
        try {
            ...
            horaMenos2Horas = Hora.restarMinutos(horaActual, 120);
            tramasDtoMaximos = tramaDao.getTramaMaximos(new Date(), new Date(), horaMenos2Horas, horaActual);
            tramasDtoMinimos = tramaDao.getTramaMinimos(new Date(), new Date(), horaMenos2Horas, horaActual);
            tramaDtoPromedios = tramaDao.getTramaPromedio(new Date(), new Date(), horaMenos2Horas, horaActual);
        } catch (ParseException ex) {
            Logger.getLogger(Job2Horas.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        for (int i = 0; i < tramasDtoMaximos.size(); i++) {
            ...
            if (tramaPromedios.getNodo().getId() == tramaMaximos.getNodo().getId()
                ...
                tramaProcesada.setTipoProcesamiento(new TipoProcesamiento(TipoProcesamiento.HORAS2));
